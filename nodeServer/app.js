...
const { auth_Exception } = require('./config/config');
app.set("basic_location" , property.basic_location);
...
app.use(function(req, res, next) {
  ...
  //mobile Main 
  if( ((req.url).substring( 0 , (req.url).indexOf("?"))) == "/m/main"){
    chk_result=true;
  };


  res.locals.gUserNm = (req.session.user) ? (req.session.user).USER_NAME : "";
  res.locals.gUserId = (req.session.user) ? (req.session.user).USER_ID : "";

  if(! chk_result ){
    //Auth Check failed

    if( ((req.url).substring( 0 , (req.url).indexOf("?"))) == "/m/"){
      throw new Error("모바일 에러");
    };
